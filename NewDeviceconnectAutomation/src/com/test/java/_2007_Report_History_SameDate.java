...
public class _2007_Report_History_SameDate extends ScriptFuncLibrary {
...
//	String reportOf[] = {"application","device","user"};
	...
	public final void testScript()
	{
		try
		{
			...
			apiReporter.apiScriptHeading("_2007_Report_History_SameDate");
			...
			for (String reportFor : reportOf)
			{
				...
				String sameDate=todaydt;
				//Set Customize date.
				isEventSuccessful=setCustomizedDates_SFL(sameDate,sameDate);
				PerformAction("browser","waitforpagetoload");
				isEventSuccessful=selectEvents_History("Application Launched,Application Install");
				PerformAction("browser","waitforpagetoload");
				
				
				if ((PerformAction(dicOR.get("eleNoHistoryErrorMessage"), Action.isDisplayed)))
				{
					sameDate=twoDaysAgodt;
					//Set Customize date.
					isEventSuccessful=setCustomizedDates_SFL(sameDate,sameDate);
					isEventSuccessful=selectEvents_History("Application Launched,Application Install");
					
					if ((PerformAction(dicOR.get("eleNoHistoryErrorMessage"), Action.isDisplayed)))
						throw new RuntimeException("No history available for today or 2 days ago date");
				}
				
				PerformAction("browser","waitforpagetoload");
				
				//Getting App ID of the application in the first history record with App Name  -- To get the API response having atleast 1 record.
				if (!(PerformAction(dicOR.get("eleNoHistoryErrorMessage"), Action.isDisplayed)))
				{
...
//				String component= "Report", parameterList="type,"+reportFor+"Id,from,to", valueList="history,"+Id+","+apiMethods.convertDateLocalTimezoneToUTC(twoDaysAgodt).replace(":", "%3A").replace(" ", "%20")+".000000Z,"+apiMethods.convertDateLocalTimezoneToUTC(todaydt).replace(":", "%3A").replace(" ", "%20")+".000000Z"; //declaration and intialization of curl command varaiables. 
		        ...
		        String component= "Report", parameterList="type,"+reportFor+"Id,fromDate,toDate", valueList="history,"+Id+","+sameDate+","+sameDate; //declaration and intialization of curl command varaiables.
