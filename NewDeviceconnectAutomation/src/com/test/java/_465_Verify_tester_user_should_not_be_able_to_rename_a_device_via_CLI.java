...
public class _465_Verify_tester_user_should_not_be_able_to_rename_a_device_via_CLI extends ScriptFuncLibrary
{
	...
	public final void testScript() throws InterruptedException, IOException
	{
		...
		if (isEventSuccessful)
		{	
	     	 Values = ExecuteCLICommand("renametester", "iOS", EmailAddress, Password, "", "");
			 ...
			 if (cmdText.contains("User does not have required entitlement"))
				{
				   ...
				   strActualResult = "Tester can not rename device via CLI";
				}
				else
				{
		...
		reporter.ReportStep("Tester should not be able to rename device via CLI" , "Tester should not be able to rename device", strActualResult, isEventSuccessful);
		if (!isEventSuccessful)
		{
			return;
		}
