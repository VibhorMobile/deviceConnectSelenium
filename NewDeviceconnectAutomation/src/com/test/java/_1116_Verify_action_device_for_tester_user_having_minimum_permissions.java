 ...
 * Coverage from Jira Case: Available and inUse devices
 */

public class _1116_Verify_action_device_for_tester_user_having_minimum_permissions extends ScriptFuncLibrary {
	...
	public final void testScript() throws InterruptedException, IOException
	{
		try{
			...
			apiReporter.apiScriptHeading("_1116_Verify_action_device_for_tester_user_having_minimum_permissions");
			...
			if(isEventSuccessful){
				...
				String []propertyName={"reboot","retain","disable","release"};
				//String []propertyValue={"editedName","125","editedNotes","false","invalid","true","false"};
				for(int i=0;i<=propertyName.length-1;i++){
					apiReporter.apiHeading2("Perform "+propertyName[i]+" action on online device "+deviceName);
					...
					//************Create cURL command********************************
					createAndexecuteCurlCommand(deviceID, propertyName[i]);
					...
					if(!propertyName[i].equals("release")){
					...
					isEventSuccessful=verifyFromUI(propertyName[i]);
				}
			}
			...
			//****************************************************************** Scenario's for inuse(Same/Othere User) devices starts here****************************************
			// Reboot,Retain,Release,Disable//
			//*********************************************************************************************************************************************************************
					...
					if(isEventSuccessful){
						...
						if(isEventSuccessful){
							...
							for(int j=0;j<=3;j++){
								...
								//****************** Verify Details from UI*************************
								if(!(propertyName[j].equals("release") && devices[i].equals(deviceNameSameUser)) ||(!propertyName[j].equals("retain"))){
