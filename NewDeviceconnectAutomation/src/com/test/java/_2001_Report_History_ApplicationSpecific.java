 ...
 * Creation Date: 21 Aug 2016
 * Last Modified Date: 25 Oct 2016 - Added not null verifications for all records returned by API 
 * Pre-requisite: Server should have atleast 1 android and iOS application
 * Admin user can only be used
 */

public class _2001_Report_History_ApplicationSpecific extends ScriptFuncLibrary {
	...
	public final void testScript()
	{
		try
		{
			...
			apiReporter.apiScriptHeading("_2001_Report_History_ApplicationSpecific");
			String Id="";
			...
			if (!(PerformAction(dicOR.get("eleNoHistoryErrorMessage"), Action.Exist)))
			{
...
//				String component= "Report", parameterList="type,"+reportFor+"Id,from,to", valueList="history,"+Id+","+apiMethods.convertDateLocalTimezoneToUTC(twoDaysAgodt).replace(":", "%3A").replace(" ", "%20")+".000000Z,"+apiMethods.convertDateLocalTimezoneToUTC(todaydt).replace(":", "%3A").replace(" ", "%20")+".000000Z"; //declaration and intialization of curl command varaiables. 
			...
			expectedAppSpecificRecordCount=apiMethods.getHistoryRecordFromUIForSpecificApp(Id, udeviceNameRecords, uIEventDateRecords, uIPerfomedByNameRecords, uIAppNameRecords);
