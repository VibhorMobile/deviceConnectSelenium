...
public class _2005_Report_History_UserSpecific extends ScriptFuncLibrary {
...
//	String reportOf[] = {"application","device","user"};
	...
	public final void testScript()
	{
		try
		{
			...
			apiReporter.apiScriptHeading("_2005_Report_History_UserSpecific");
			...
			for (String reportFor : reportOf)
			{
				...
				//Getting a date 3 days ago for defining the date range
		      ...
		      //**************************************************************************//
		        ...
		        String FiveDaysAgodt = dateFormat.format(twoDayAgodate);
				...
				if(isEventSuccessful)
				{
					isEventSuccessful=setCustomizedDates(FiveDaysAgodt,todaydt);
					if(isEventSuccessful)
					{
...
//				String component= "Report", parameterList="type,"+reportFor+"Id,from,to", valueList="history,"+Id+","+apiMethods.convertDateLocalTimezoneToUTC(FiveDaysAgodt).replace(":", "%3A").replace(" ", "%20")+".000000Z,"+apiMethods.convertDateLocalTimezoneToUTC(todaydt).replace(":", "%3A").replace(" ", "%20")+".000000Z"; //declaration and intialization of curl command varaiables. 
		        
		        String component= "Report", parameterList="type,"+reportFor+"Id,fromDate,toDate", valueList="history,"+Id+","+FiveDaysAgodt+","+todaydt; //declaration and intialization of curl command varaiables.
				...
				isEventSuccessful=apiMethods.verifyfieldsNotNullInAPIResponse(jsonResponse, "id,eventDate,performedById", "history");
				//--------------------------------------------------------------------------
...
//					apiReporter.apiAddBlock("Actual (from API): " +actdeviceName+" "+acteventDate+" "+actperformedByName+" "+actappName);
					...
					if ((expappName.equals(actappName) || actappName==null) && (expdeviceName.equals(actdeviceName) || actdeviceName==null) && (expeventDate.equals(acteventDate)) && (expperformedByName.equals(actperformedByName)))
					{
