 ...
 * Jira Test Case Id: QA-1993
 * Coverage from Jira Case: Complete
 */
public class _1105_API_Verify_setting_port_mode_by_device_id_works_using_Rest_API extends ScriptFuncLibrary {
	...
	String []propValue={"charge","off","data"};
	...
	public final void testScript()	{
		try{
			...
			apiReporter.apiScriptHeading("_1105_API_Verify_setting_port_mode_by_device_id_works_using_Rest_API");
			...
			for(int j=0; j<=1; j++)
			{
				...
				if(devicesErrorMsg.contains("No devices match your filter criteria.")){
					...
					for (String valueOfProp: propValue)
					{
						...
						apiReporter.apiHeading2(" Update port mode to --"+valueOfProp+"-- for "+deviceType[j]+" Device:"+deviceName+"using device id:"+deviceID);
						//Create the curl command to update user details with id
						String component= "Device/UsbPort", parameterList="verb"+",id"+","+updateProp, valueList="update"+","+deviceID+","+valueOfProp; //declaration and intialization of curl command varaiables.
