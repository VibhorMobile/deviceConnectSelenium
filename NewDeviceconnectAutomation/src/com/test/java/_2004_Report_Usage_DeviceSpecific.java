...
public class _2004_Report_Usage_DeviceSpecific extends ScriptFuncLibrary {
...
//	String reportOf[] = {"application","device","user"};
	...
	public final void testScript()
	{
		try
		{
			...
			apiReporter.apiScriptHeading("_2004_Report_Usage_DeviceSpecific");
			...
			for (String reportFor : reportOf)
			{
				...
				//Getting device ID  in the first usage record -- To get the API response having atleast 1 record.
...
//				String component= "Report", parameterList="type,"+reportFor+"Id,from,to", valueList="history,"+Id+","+apiMethods.convertDateLocalTimezoneToUTC(twoDaysAgodt).replace(":", "%3A").replace(" ", "%20")+".000000Z,"+apiMethods.convertDateLocalTimezoneToUTC(todaydt).replace(":", "%3A").replace(" ", "%20")+".000000Z"; //declaration and intialization of curl command varaiables. 
		        ...
		        String component= "Report", parameterList="type,"+reportFor+"Id,fromDate,toDate", valueList="usage,"+Id+","+twoDaysAgodt+","+todaydt; //declaration and intialization of curl command varaiables.
				...
				isEventSuccessful=apiMethods.verifyfieldsNotNullInAPIResponse(jsonResponse, "id,deviceId,deviceName,operatingSystem,operatingSystemVersion,startDate,endDate,duration", "usage");
				//--------------------------------------------------------------------------
