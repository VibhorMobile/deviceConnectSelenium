...
/*
 * Jira Test Case Id: QA-1899
 */
public class _635_Verify_error_messgae_for_user_do_not_have_entitlements_to_reset_device_state extends ScriptFuncLibrary
{
	...
	public final void testScript() throws IOException 
	{
		...
		//**************************************************************************//
		// Step 5 : Connect to an iOS device and verify deviceViewer is launched on the windows desktop.
		//**************************************************************************//
		if (isEventSuccessful)
		{	
		...
		//**************************************************************************//
		Values = ExecuteCLIResetCommands("resetrebootuninstall", "Android", testerEmailAddress, testerPassword, deviceName, "","");
		isEventSuccessful = (boolean)Values[2];
		outputText=(String)Values[0];
		if (isEventSuccessful && outputText.contains("User does not have required entitlement"))
		{
			strActualResult = "User does not have required entitlement";
		}
		else
		{
			strActualResult = "ExecuteCLICommand---" + strErrMsg_AppLib;
		...
		//*************************************************************//                     
		// Step 8 : login to deviceConnect with admin user.
		//*************************************************************//                     
		isEventSuccessful = Login();
		
		//*************************************************************//                     
		// Step 9 : Go to user page and set entitlement .
		//*************************************************************// 
		isEventSuccessful=GoToUsersPage();
		if(isEventSuccessful)
		{
		...
		//*************************************************************//                     
		// Step 11 : login to deviceConnect with test user.
		//*************************************************************//  
		if(isEventSuccessful)
		{
			strstepDescription = "Login to deviceConnect with valid test user.";
		...
		//**************************************************************************//
		// Step 12 : Connect to an iOS device and verify deviceViewer is launched on the windows desktop.
		//**************************************************************************//
		if (isEventSuccessful)
		{	
			Values = ExecuteCLICommand("run", "Android", testerEmailAddress, testerPassword, "", "");
		...
		//**************************************************************************//
		// Step 13 : Reset device via CLI command.
		//**************************************************************************//
		Values = ExecuteCLIResetCommands("resetrebootuninstall", "Android", testerEmailAddress, testerPassword, deviceName, "","");
		isEventSuccessful = (boolean)Values[2];
		outputText=(String)Values[0];
